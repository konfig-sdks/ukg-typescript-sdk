...
import { readableStreamToString, UkgError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new UkgError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new UkgError(e, e.response?.data)
                }
                if (innerError instanceof UkgError) {
                    // Got "UkgError" from the above try block
                    throw innerError;
                }
